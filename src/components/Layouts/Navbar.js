...
export const Navbar = ({Deals}) => {
    ...
    // let count=localStorage.getItem("cart_count")? localStorage.getItem('cart_count'):0
            ...
            <div className='container-fluid bg-dark'>
                <div className='row'>
                    <div className='col-md-3 text-center'>
                        <Link className="navbar-brand text-white fs-3 fw-bold" to="/">Khem ko Shop</Link>
                    </div>
                    ...
                    <div className='col-md-3 d-flex justify-content-evenly'>
{
    !user &&
    <>
        <Link to='/signin'><i className='bi bi-box-arrow-in-left m-2 fs-2 p-2  text-white'></i></Link>
        <Link to='/register'><i className='bi bi-person-plus m-2 fs-2 p-2 text-white'></i></Link>
    </>
}
{
    user && user.role === 1 &&
...
}
{
    ...
    <Link to='/cart' className='position-relative'><i className='bi bi-cart fs-2 p-2 text-white'></i>
    <span className='bg-warning position-absolute top-25 left-100 rounded-circle px-1'>{count}</span></Link>
    </>
}
{
    user &&
